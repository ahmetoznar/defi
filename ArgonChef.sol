...
contract ArgonChef is Ownable, ApproverRole {
    using SafeMath for uint256;
    ...
    function stopReward() public onlyApprover {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 calcTargPerBlock = rewardPerBlock;
            ...
            if (now < secondYear) {
                calcTargPerBlock = rewardPerBlock;
            } else if (now < thirdYear) {
                calcTargPerBlock = rewardPerBlock.div(2);
            } else if (now < forthYear) {
                calcTargPerBlock = rewardPerBlock.div(4);
            } else {
                calcTargPerBlock = rewardPerBlock.div(8);
            }
            
            uint256 cakeReward = multiplier.mul(calcTargPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyApprover {
        require(_amount < rewardToken.balanceOf(address(this)), 'not enough token');
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }

    function setMinStakingTime(uint256 _stakingTime) public onlyApprover{
    ...
    function setRewardPerBlock(uint256 _rewardPerBlock) public onlyApprover{
        rewardPerBlock = _rewardPerBlock;
    }
